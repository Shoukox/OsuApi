...
namespace OsuApi.V2.Clients.Rankings
{
    public sealed class RankingsClient : Client
    {
        ...
        /// <returns></returns>
        public async Task<Models.Rankings?> GetRanking(string mode, string rankingType, GetRankingQueryParameters parameters, CancellationToken? cancellationToken = null)
        {
            ApiUtility.ThrowIfParameterValueIsNotOfType(mode, typeof(Ruleset));
            
            var response = await Api.MakeRequestAsync<Models.Rankings>(
                url: ApiV2.ApiMainFunctionsBaseAddress + $"/rankings/{mode}/{rankingType}",
                HttpMethod.Get,
